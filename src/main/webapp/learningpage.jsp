...
<body>
		...
		<nav class="navbar navbar-dark bg-dark fixed-top">
			<div class="container-fluid">
				...
				<div class="ms-auto p-1">
					<button id="speakButton" class="btn btn-danger btn-rounded"
						data-mdb-ripple-init onclick="toggleSpeech()">Read
						Content</button>
				</div>
					...
					<div class="offcanvas-body">
						<ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
							...
							try {
								...
								con = DriverManager.getConnection("jdbc:mysql://localhost/lp", "root", "akshay@26");
							...
							<li class="nav-item"><a class="nav-link active"
								aria-current="page"
								href="./learningpage.jsp?id=<%=rs.getInt("id")%>&courseid=<%=courseid%>&coursename=<%=request.getParameter("coursename")%>"><%=rs.getString("title")%></a>
							...
							<li class="nav-item">
								...
								<a class="nav-link active quiz-link" data-bs-toggle="modal"
								data-bs-target="#quizInstructionsModal">Quiz</a>
							</li>
					</div>
				</div>
			</div>
	...
	<!-- Modal structure -->
	<div class="modal fade" id="quizInstructionsModal" tabindex="-1"
		aria-labelledby="quizInstructionsModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="quizInstructionsModalLabel">Quiz
						Instructions</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal"
						aria-label="Close"></button>
				</div>
				<div class="modal-body">
					<h3 style="color: red">Before You Begin, Please Read the
						Following Instructions Carefully:</h3>
					<ol>
						<li>There are a total of 10 questions.</li>
						<li>You will have 30 seconds to answer each question.</li>
						<li>Read each question thoroughly, focus, and avoid
							distractions.</li>
						<li>Ensure your internet connection is stable to receive AI
							feedback promptly.</li>
					</ol>
				...
				<div class="modal-footer">
					...
					<a
						href="QuizServlet?course=<%=request.getParameter("coursename")%>&courseid=<%=courseid%>"
	...
	<div class="navigation-buttons">
		...
		%>
		<a
			href="./learningpage.jsp?id=<%=prevTopicId%>&courseid=<%=courseid%>&coursename=<%=request.getParameter("coursename")%>"
			class="btn btn-primary prev-button">Previous</a>
		<%
		}
		%>
		<%
		...
		%>
		<a
			href="./learningpage.jsp?id=<%=nextTopicId%>&courseid=<%=courseid%>&coursename=<%=request.getParameter("coursename")%>"
			class="btn btn-primary next-button">Next</a>
		<%
		}
		%>
	</div>
